...
import com.knowledge.api.base.config.jwt.annotation.PassToken;
		...
		if (method.isAnnotationPresent(UserLoginToken.class)) {
			...
			if (userLoginToken.required()) {
				...
				if (Validator.isNullOrEmpty(token)) {
					throw new RuntimeException(EnumResult.ERROR_NOT_TOKEN.getMsg());
				}
				// 获取 token 中的 user id
				String userId;
				try {
				...
				if (Validator.isNullOrEmpty(user)) {
					throw new RuntimeException(EnumResult.DEFIN_ACCOUNT.getMsg());
				}
				// 验证 token
